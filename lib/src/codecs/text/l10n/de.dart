...
class RruleL10nDe extends RruleL10n {
  ...
  String until(DateTime until, Frequency frequency, {DateFormat? dateFormat}) {
    final untilString = formatWithIntl(() => dateFormat?.format(until) ?? DateFormat.yMMMMEEEEd().add_jms().format(until));
  ...
  String onDaysOfWeek(
    ...
    final daysWithSuffix = frequency == null ? days.replaceAll(RegExp(r'tag\b'), 'tags').replaceAll('Mittwoch', 'Mittwochs') : days;
    ...
    final prefix = (frequency != null ? _onFrequencyVariant : _onVariant)(variant);
